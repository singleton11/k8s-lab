...
APP?=k8s-lab
PROJECT=github.com/singleton11/k8s-lab
...
K8SLAB_LOCAL_HOST?=0.0.0.0
K8SLAB_LOCAL_PORT?=8080
K8SLAB_LOG_LEVEL?=0
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${K8SLAB_LOCAL_PORT}:${K8SLAB_LOCAL_PORT} \
		-e "K8SLAB_LOCAL_HOST=${K8SLAB_LOCAL_HOST}" \
		-e "K8SLAB_LOCAL_PORT=${K8SLAB_LOCAL_PORT}" \
		-e "K8SLAB_LOG_LEVEL=${K8SLAB_LOG_LEVEL}" \
